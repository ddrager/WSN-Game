...
implementation
{
  ...
  uint16_t ledTime, curVoltage; 
  uint16_t localRep, rootRep;
...
void randomGenerator() {
  ...
  /* 
   * function to send new TheftSend Packet. Input: Voltage 
   */
  void sendPacket() {
	...
	if (newAlert != NULL) {
		...
		newAlert->voltageData = curVoltage;
		newAlert->eventId = eventId;
		eventId = eventId + 1;
...
/* Legacy, we no longer use Read */
event void Read.readDone ( error_t result, uint16_t val ) { }

/* Legacy, we no longer use AlertRoot */
event void AlertRoot.sendDone(message_t *msg, error_t ok) { }


/* This is called whenever the mote received an AM packet from the network */
